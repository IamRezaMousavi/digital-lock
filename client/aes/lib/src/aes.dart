...
class Aes {
  ...
  Uint8List aesEncrypt(Uint8List data) {
    ...
    final encData = Uint8List(data.length); // returned cipher text;
    var t = Uint8List(16); // 16-byte block to hold the temporary input of the cipher
    var block16 = Uint8List.fromList(_aesIV); // 16-byte block to hold the temporary output of the cipher
  ...
  Uint8List aesDecrypt(Uint8List data) {
    ...
    var decData = Uint8List(data.length); // returned decrypted data;
    final t = Uint8List(16); // 16-byte block
    Uint8List x_block;
    var block16 = Uint8List.fromList(_aesIV); // 16-byte block to hold the temporary output of the cipher
  ...
  Uint8List aesEncryptBlock(Uint8List data) {
    ...
    final encBlock = Uint8List(16); // 16-byte string
    int i;
  ...
  Uint8List aesDecryptBlock(Uint8List data) {
    ...
    final decBlock = Uint8List(16); // 16-byte string
    int i;
  ...
  void _invShiftRows() {
    final temp = List<int?>.filled(_Nb, null, growable: false);
    for (var i = 1; i < 4; ++i) {
      for (var j = 0; j < _Nb; ++j) {
        temp[(i + j) % _Nb] = _s[i][j];
      }
