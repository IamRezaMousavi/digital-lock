...
enum AesFileMode {
  ...
  /// exception with [AesFileExceptionType.destFileExists] type.
...
class AesFile {
  ...
  AesFile.init(Uint8List? passBytes, AesFileMode? mode, Map<String, List<int>>? userdata) {
    _passBytes = passBytes != null ? Uint8List.fromList(passBytes) : Uint8List(0);
    _owMode = mode;
    if (userdata == null || userdata.isEmpty) {
      _userdata = {'CREATED_BY': 'Dart AES Library'.toUtf8Bytes()};
    } else {
  ...
  String encryptDataToFileSync(Uint8List srcData, String destFilePath) {
    ...
    _encryptDataAndCalculateHmac(srcData, _dp[_Data.key2]!, _dp[_Data.iv2]!, AesMode.cbc);
    _log('HMAC2', _dp[_Data.hmac2]);

    // Write encrypted data to file

    destFilePath = _modifyDestinationFilenameSync(destFilePath);
    final outFile = File(destFilePath);
  ...
  String encryptFileSync(String srcFilePath, String destFilePath) {
    ...
    final inFileLength = inFile.lengthSync();
    final srcData = Uint8List(inFileLength);
    ...
    destFilePath = _makeDestFilenameFromSource(srcFilePath, destFilePath, _Action.encrypting);
    destFilePath = _modifyDestinationFilenameSync(destFilePath);
    final outFile = File(destFilePath);
    RandomAccessFile raf;
    try {
  ...
  Uint8List decryptDataFromFileSync(String srcFilePath) {
    ...
    final keys = _readKeysSync(f);

    final encrypted_data = _readChunkBytesSync(f, f.lengthSync() - f.positionSync() - 33, 'encrypted data');
    _log('ENCRYPTED DATA', encrypted_data);

    final file_size_modulo = _readChunkIntSync(f, 1, 'file size modulo')!;
    ...
    final hmac_2 = _readChunkBytesSync(f, 32, 'HMAC 2');
    _log('HMAC_2', hmac_2);
    f.closeSync();
    ...
    final decrypted_data_full = _aes.aesDecrypt(encrypted_data);

    final decrypted_data = decrypted_data_full.buffer
        .asUint8List(0, decrypted_data_full.length - (file_size_modulo == 0 ? 0 : 16 - file_size_modulo));
  ...
  Future<Uint8List> decryptDataFromFile(String srcFilePath) async {
    ...
    final keys = await _readKeys(f);

    final encrypted_data = await _readChunkBytes(f, f.lengthSync() - f.positionSync() - 33, 'encrypted data');
    _log('ENCRYPTED DATA', encrypted_data);

    final file_size_modulo = await _readChunkInt(f, 1, 'file size modulo');
    _log('FILE SIZE MODULO', file_size_modulo);
    if (file_size_modulo < 0 || file_size_modulo >= 16) {
      throw AesCryptDataException('Invalid file size modulo: $file_size_modulo');
    }

    final hmac_2 = await _readChunkBytes(f, 32, 'HMAC 2');
    _log('HMAC_2', hmac_2);
    f.closeSync();
  ...
  String decryptFileSync(String srcFilePath, String destFilePath) {
    ...
    try {
      ...
      throw FileSystemException('Failed to open file $srcFilePath for reading.', e.path, e.osError);
    }
    ...
    _log('WRITE', 'Started');
    destFilePath = _makeDestFilenameFromSource(srcFilePath, destFilePath, _Action.decrypting);
    destFilePath = _modifyDestinationFilenameSync(destFilePath);
    final outFile = File(destFilePath);
    RandomAccessFile raf;
    try {
    ...
    try {
      raf.writeFromSync(
          decrypted_data_full, 0, decrypted_data_full.length - (file_size_modulo == 0 ? 0 : 16 - file_size_modulo));
    } on FileSystemException catch (e) {
      raf.closeSync();
...
//****************************************************************************
  ...
  Uint8List sha256(Uint8List data, [Uint8List? hmacIpad]) {
    ...
    final lengthPadded = length + 8 - ((length + 8) & 0x3F) + 64;
    final lengthToWrite = (hmacIpad == null ? length : length + 64) * 8;
    ...
    if (length < lengthPadded - 64) {
      ...
      chunkLastPre = Uint8List(64)
        ...
        ..[length & 0x3F] = 0x80;
      chunkLast = Uint8List(64)..buffer.asByteData().setInt64(56, lengthToWrite);
    } else {
    ...
    for (var n = 0; n < chanksToProcess; n += 64) {
    ...
    final hash = Uint32List.fromList([_h0!, _h1!, _h2!, _h3!, _h4!, _h5!, _h6!, _h7!]);
    for (var i = 0; i < 8; ++i) {
...
//****************************************************************************
  ...
  Uint8List hmacSha256(Uint8List key, Uint8List data) {
    ...
    final i_pad = Int32x4List(4);
    final o_pad = Int32x4List(6);
    ...
    final temp = sha256(data, i_pad.buffer.asUint8List());
    final buff2 = o_pad.buffer.asUint8List()..setRange(64, 96, temp);
    return sha256(buff2);
  }
...
//****************************************************************************
  ...
  void _encryptDataAndCalculateHmac(Uint8List srcData, Uint8List key, Uint8List iv, AesMode mode) {
    ...
    final i_pad = Int32x4List(4);
    final o_pad = Int32x4List(6);
    final keyForHmac = Uint8List(64)..setAll(0, _aes.getKey());
    for (var i = 0; i < 4; i++) {
    ...
    ByteData chunk;
    final length = srcData.length; // unencrypted data length
    final lengthPadded16 = length + ((length % 16 == 0) ? 0 : 16 - length % 16); // AES encrypted data length
    final lengthPadded64 = lengthPadded16 + 8 - ((lengthPadded16 + 8) & 0x3F) + 64; // length for SHA256
    final lengthToWrite = (lengthPadded16 + 64) * 8;
    _h0 = 0x6a09e667;
    _h1 = 0xbb67ae85;
    _h2 = 0x3c6ef372;
    _h3 = 0xa54ff53a;
    ...
    final encData = Uint8List(lengthPadded16);
    final t = Uint8List(16);
    var block16 = Uint8List.fromList(_aes.getIV());
    ...
    final lastChunk = Uint8List(64)..setAll(0, encData.buffer.asUint8List(n));
    chunk = lastChunk.buffer.asByteData()
      ..setUint8(i - n, 0x80)
      ..setInt64(56, lengthToWrite);
    for (var i = 0; i < 16; ++i) {
      _chunkBuff[i] = chunk.getUint32(i * 4);
    }
    _processChunk();
    ...
    final buff2 = o_pad.buffer.asUint8List()..setRange(64, 96, hash.buffer.asUint8List());
...
//****************************************************************************
  ...
  Uint8List _keysJoin(Uint8List iv, Uint8List pass) {
    var key = Uint8List(32)
      ..setAll(0, iv);
    final len = 32 + pass.length;
    final buff = Uint8List(len);
    for (var i = 0; i < 8192; i++) {
      buff
        ..setAll(0, key)
        ..setRange(32, len, pass);
      key = sha256(buff);
    }
    return key;
  }
  ...
  void _validateHMAC(Uint8List key, Uint8List data, Uint8List hash, _HmacType ht) {
    final calculated = hmacSha256(key, data);
  ...
  Uint8List _getUserDataAsBinary() {
    ...
    int len;
    for (final me in _userdata.entries) {
  ...
  void _createDataParts() {
    ...
    _dp[_Data.iv1] = AesCrypt().createIV();
    ...
    _dp[_Data.iv2] = AesCrypt().createIV();
    _log('IV_2', _dp[_Data.iv2]);
    _dp[_Data.key2] = AesCrypt().createKey();
    ...
    _dp[_Data.enckeys] = _aes.aesEncrypt(_dp[_Data.iv2]!.addList(_dp[_Data.key2]!));
  ...
  Map<_Data, Uint8List> _readKeysSync(RandomAccessFile f) {
    final keys = <_Data, Uint8List>{};

    final head = _readChunkBytesSync(f, 3, 'file header');
    final expected_head = Uint8List.fromList([65, 69, 83]);
    ...
    final version_chunk = _readChunkIntSync(f, 1, 'version byte');
    ...
    // User data
    if (version_chunk == 2) {
      var ext_length = _readChunkIntSync(f, 2, 'extension length');
      while (ext_length != 0) {
    ...
    final iv_1 = _readChunkBytesSync(f, 16, 'IV 1');
    _log('IV_1', iv_1);

    final key_1 = _keysJoin(iv_1, _passBytes!);
    ...
    final enc_keys = _readChunkBytesSync(f, 48, 'Encrypted Keys');
    _log('ENCRYPTED KEYS', enc_keys);

    // HMAC
    final hmac_1 = _readChunkBytesSync(f, 32, 'HMAC 1');
    _log('HMAC_1', hmac_1);

    _validateHMAC(key_1, enc_keys, hmac_1, _HmacType.HMAC1);

    _aes.aesSetParams(key_1, iv_1, AesMode.cbc);
    final decrypted_keys = _aes.aesDecrypt(enc_keys);
  ...
  Future<Map<_Data, Uint8List>> _readKeys(RandomAccessFile f) async {
    ...
    final head = await _readChunkBytes(f, 3, 'file header');
    final expected_head = Uint8List.fromList([65, 69, 83]);
    if (head.isNotEqual(expected_head)) {
      throw AesCryptDataException(
          "The chunk 'file header' was expected to be ${expected_head.toHexString()} but found ${head.toHexString()}");
    ...
    // Initialization Vector (IV) used for encrypting the IV and symmetric key
    // that is actually used to encrypt the bulk of the plaintext file.
    final iv_1 = await _readChunkBytes(f, 16, 'IV 1');
    _log('IV_1', iv_1);
    ...
    // 16 octets - initialization vector
    // 32 octets - encryption key
    final enc_keys = await _readChunkBytes(f, 48, 'Encrypted Keys');
    _log('ENCRYPTED KEYS', enc_keys);

    // HMAC
    final hmac_1 = await _readChunkBytes(f, 32, 'HMAC 1');
    _log('HMAC_1', hmac_1);
  ...
  String _makeDestFilenameFromSource(String srcFilePath, String destFilePath, _Action action) {
      ...
      case AesFileMode.warn:
        if (_isPathExistsSync(destFilePath)) {
          throw AesCryptException(
              ...
              AesFileExceptionType.destFileExists);
        }
      case AesFileMode.on:
        if (_isPathExistsSync(destFilePath) && FileSystemEntity.typeSync(destFilePath) != FileSystemEntityType.file) {
          throw AesCryptArgumentError('Destination path $destFilePath is not a file and can not be overwritten.');
        }
        //File(destFilePath).deleteSync();
      case null:
        throw Exception('Overwrite mode not set. Use AesCryptOwMode.rename, AesCryptOwMode.warn or AesCryptOwMode.on.');
    }
  ...
  Future<String> _modifyDestinationFilename(String destFilePath) async {
    switch (_owMode) {
      ...
      case AesFileMode.on:
        if ((await _isPathExists(destFilePath)) &&
            (FileSystemEntity.typeSync(destFilePath)) != FileSystemEntityType.file) {
          throw AesCryptArgumentError('Destination path $destFilePath is not a file and can not be overwritten.');
        }
  ...
  bool _isPathExistsSync(String path) => FileSystemEntity.typeSync(path) != FileSystemEntityType.notFound;

  Future<bool> _isPathExists(String path) async => (await FileSystemEntity.typeSync(path)) != FileSystemEntityType.notFound;
}
