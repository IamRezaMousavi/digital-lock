...
static inline void consume_chunk(uint32_t *h, const uint8_t *p) {
   ...
   *
   * This optimization was not there initially and the rest of the comments about w[64] are kept in
   * their initial state.
   */

  /*
   * create a 64-entry message schedule array w[0..63] of 32-bit words (The initial values in
   * w[0..63] don't matter, so many implementations zero them here) copy chunk into first 16 words
   * w[0..15] of the message schedule array
   */
  uint32_t w[16];
...
void sha_256_write(struct Sha_256 *sha_256, const void *data, size_t len) {
  ...
  /*
   * The following cast is not necessary, and could even be considered as poor practice. However, it
   * makes this file valid C++, which could be a good thing for some use cases.
   */
  const uint8_t *p = (const uint8_t *)data;

  while (len > 0) {
    /*
     * If the input chunks have sizes that are multiples of the calculation chunk size, no copies
     * are necessary. We operate directly on the input data instead.
     */
...
uint8_t *sha_256_close(struct Sha_256 *sha_256) {
  ...
  /*
   * The current chunk cannot be full. Otherwise, it would already have been consumed. I.e. there is
   * space left for at least one byte. The next step in the calculation is to add a single one-bit
   * to the data.
   */
  *pos++ = 0x80;
  --space_left;
  ...
  /*
   * Now, the last step is to add the total data length at the end of the last chunk, and zero
   * padding before that. But we do not necessarily have enough space left. If not, we pad the
