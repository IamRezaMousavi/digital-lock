...
void KeyPad_Init(void) {
  ...
  for (uint8_t i = 0; i < KeyPad.ColumnSize; i++) {
    ...
    HAL_GPIO_WritePin(
        (GPIO_TypeDef *)_KEYPAD_COLUMN_GPIO_PORT[i], _KEYPAD_COLUMN_GPIO_PIN[i], GPIO_PIN_SET
    );
  }
...
uint16_t KeyPad_Scan(void) {
  ...
  for (uint8_t c = 0; c < KeyPad.ColumnSize; c++) {
    ...
    HAL_GPIO_WritePin(
        (GPIO_TypeDef *)_KEYPAD_COLUMN_GPIO_PORT[c], _KEYPAD_COLUMN_GPIO_PIN[c], GPIO_PIN_RESET
    );
    _KEYPAD_DELAY(5);
    for (uint8_t r = 0; r < KeyPad.RowSize; r++) {
      if (HAL_GPIO_ReadPin((GPIO_TypeDef *)_KEYPAD_ROW_GPIO_PORT[r], _KEYPAD_ROW_GPIO_PIN[r])
          == GPIO_PIN_RESET) {
        ...
        if (HAL_GPIO_ReadPin((GPIO_TypeDef *)_KEYPAD_ROW_GPIO_PORT[r], _KEYPAD_ROW_GPIO_PIN[r])
          ...
          key |= 1 << c;
          key |= 1 << (r + 8);
          while (HAL_GPIO_ReadPin((GPIO_TypeDef *)_KEYPAD_ROW_GPIO_PORT[r], _KEYPAD_ROW_GPIO_PIN[r])
                 == GPIO_PIN_RESET)
            _KEYPAD_DELAY(5);
          return key;
        }
